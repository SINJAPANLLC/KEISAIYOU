...
import { sendEmail, sendAdminNotification, sendLineMessage, isEmailConfigured, isLineConfigured, replaceTemplateVariables } from "./notification-service";
...
export async function registerRoutes(
  ...
  app.post("/api/register", async (req, res) => {
    try {
      ...
      setImmediate(() => {
        void sendAdminNotification("【KEI SAIYOU】新規企業登録のお知らせ", {
          title: "新しい企業登録がありました",
          badge: { text: "新規登録" },
          rows: [
            { label: "会社名", value: parsed.data.companyName || "未入力" },
            { label: "担当者", value: parsed.data.contactName || "未入力" },
            { label: "メールアドレス", value: parsed.data.email || "未入力" },
            { label: "電話番号", value: parsed.data.phone || "未入力" },
            { label: "都道府県", value: parsed.data.prefecture || "未入力" },
            { label: "登録種別", value: parsed.data.userType || "未入力" },
            { label: "ユーザーID", value: user.id },
          ],
          ctaText: "管理画面で確認する",
          ctaUrl: "https://keisaiyou-sinjapan.com/admin/users",
        }).catch((err) => {
          console.error("Admin registration notification failed:", err);
        });
      });
  ...
  // ─────────────────────────────────────────────
  // KEI MATCH → KEI SAIYOU ユーザー同期
  // ─────────────────────────────────────────────
  app.post("/api/admin/keimatch/sync-users", requireAdmin, async (req, res) => {
    const kmUrl = process.env.KEIMATCH_DATABASE_URL;
    if (!kmUrl) return res.status(500).json({ message: "KEIMATCH_DATABASE_URL が設定されていません" });
    let kmPool: any = null;
    try {
      const pg = await import("pg");
      kmPool = new pg.default.Pool({ connectionString: kmUrl });
      const { rows: kmUsers } = await kmPool.query(
