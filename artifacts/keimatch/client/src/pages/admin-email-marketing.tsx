...
info@keisaiyou-sinjapan.com`,
  ...
  {
    name: "フォローアップ",
    subject: "【ご確認】KEI SAIYOUのご案内、届いておりますか？",
    body: `{{companyName}} ご担当者様
...
▶「掲載翌週に応募が来た」
▶「月額ゼロなのでリスクなく試せた」
▶「採用決定まで他社の1/3のコストで済んだ」

軽貨物ドライバーの採用に特化したサービスですので、すぐに効果を実感していただけます。
...
function buildEmailHtml(subject: string, body: string, previewCompany = "サンプル株式会社"): string {
  ...
  for (const rawLine of lines) {
    ...
    const isListItem = rawLine.startsWith("▶") || rawLine.startsWith("✅") || rawLine.startsWith("☑");
    const isDivider  = rawLine.startsWith("━") || rawLine.startsWith("─");
    const isHeading  = rawLine.startsWith("■");
    const isEmpty    = rawLine.trim() === "";
    const isUrl      = rawLine.match(/^https?:\/\//);

    if (!isListItem && inList) { parts.push("</td></tr></table>"); inList = false; }

    if (isListItem) {
      const itemText = line.replace(/^[▶✅☑]\s*/, "");
      if (!inList) {
        parts.push(`<table cellpadding="0" cellspacing="0" width="100%" style="margin:6px 0;"><tr><td style="padding:0;">`);
        inList = true;
      }
      parts.push(`<table cellpadding="0" cellspacing="0" width="100%" style="margin:5px 0;"><tr>
        <td width="28" valign="top" style="padding-top:1px;">
          <div style="width:22px;height:22px;background:#fff4ee;border-radius:50%;text-align:center;line-height:22px;font-size:12px;color:#d05a2a;font-weight:bold;">✓</div>
        </td>
        <td style="padding-left:8px;font-size:14px;color:#1e293b;line-height:1.6;">${itemText}</td>
      </tr></table>`);
    } else if (isDivider) {
      parts.push(`<hr style="border:none;border-top:1px solid #e2e8f0;margin:20px 0;">`);
    } else if (isHeading) {
      const headText = line.replace(/^■\s*/, "");
      parts.push(`<div style="margin:20px 0 10px;">
        <p style="margin:0;font-size:13px;font-weight:700;color:#d05a2a;letter-spacing:0.5px;border-left:3px solid #d05a2a;padding-left:10px;">${headText}</p>
      </div>`);
    } else if (isUrl) {
      parts.push(`<div style="margin:20px 0;text-align:center;">
        <a href="${rawLine}" style="display:inline-block;background:#d05a2a;color:#fff;font-weight:bold;font-size:14px;padding:13px 36px;border-radius:6px;text-decoration:none;letter-spacing:0.3px;">無料で登録する →</a>
        <p style="margin:8px 0 0;font-size:11px;color:#94a3b8;">${rawLine}</p>
      </div>`);
    } else if (isEmpty) {
      parts.push(`<div style="height:10px;"></div>`);
    } else {
      parts.push(`<p style="margin:0 0 6px;font-size:14px;line-height:1.8;color:#334155;">${line}</p>`);
    }
  }
  if (inList) parts.push("</td></tr></table>");
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <tr><td style="padding-bottom:10px;">
        <p style="margin:0;font-size:11px;font-weight:700;letter-spacing:2px;color:#d05a2a;">▪ KEI SAIYOU</p>
      </td></tr>

      <!-- Header -->
      <tr><td style="background:linear-gradient(135deg,#c04f24 0%,#e8734a 100%);border-radius:12px 12px 0 0;padding:30px 36px 26px;">
        <p style="margin:0 0 8px;font-size:10px;color:rgba(255,255,255,0.6);letter-spacing:3px;text-transform:uppercase;font-weight:600;">SALES MESSAGE</p>
      ...
      <!-- Footer -->
      <tr><td style="background:#1e293b;border-radius:0 0 12px 12px;padding:22px 36px;">
        <table width="100%" cellpadding="0" cellspacing="0">
          <tr>
            <td>
              <p style="margin:0;font-size:13px;font-weight:700;color:#fff;letter-spacing:0.5px;">KEI SAIYOU</p>
              <p style="margin:3px 0 0;font-size:11px;color:rgba(255,255,255,0.5);">合同会社SIN JAPAN</p>
            </td>
            <td align="right">
              <p style="margin:0;font-size:11px;color:rgba(255,255,255,0.5);">info@keisaiyou-sinjapan.com</p>
            </td>
          </tr>
        </table>
      </td></tr>
